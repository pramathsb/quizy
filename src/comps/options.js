...
export function SingleOptions (props) {
    return (
        <div className="form-wrap">
            <label className={`btn btn-options ${parseInt(props.selectedAnswers[parseInt(props.qid-1)]) === props.index ? 'selected':''}`}>
...
export function MultipleOptions (props) {
    ...
    return (
        <div className="form-wrap">
            <label className={`btn btn-options ${(props.selectedAnswers[props.qid-1] ? props.selectedAnswers[props.qid-1].includes(props.index) : false) ? 'selected':''}`}>
